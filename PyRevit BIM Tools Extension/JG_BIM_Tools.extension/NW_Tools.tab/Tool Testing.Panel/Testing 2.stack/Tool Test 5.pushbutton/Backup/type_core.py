...
def diagnose_snapshot(doc):
    """Quick sanity check using the confirmed correct API (ft.AsDouble(fp))."""
    lines = []
    try:
        fm = doc.FamilyManager
        all_types  = list(fm.Types)
        all_params = list(fm.Parameters)
        lines.append("Types: " + str(len(all_types)))
        lines.append("Params: " + str(len(all_params)))
        lines.append("")

        if not all_types:
            lines.append("No types found.")
            return "\n".join(lines)

        ft = all_types[0]
        lines.append("Type: " + str(ft.Name))
        lines.append("")

        for fp in all_params[:8]:
            try:
                name = fp.Definition.Name
                st   = fp.StorageType
                is_f = fp.IsDeterminedByFormula
                expr = ""
                try:
                    expr = fp.Formula or ""
                except:
                ...
                try:
                    ...
                    lines.append("OK  " + name + " = " + repr(val) + ("  [=" + expr + "]" if expr else ""))
                except Exception as ex:
                    lines.append("ERR " + name + ": " + str(ex))
            except Exception as ex:
                lines.append("PARAM ERR: " + str(ex))
