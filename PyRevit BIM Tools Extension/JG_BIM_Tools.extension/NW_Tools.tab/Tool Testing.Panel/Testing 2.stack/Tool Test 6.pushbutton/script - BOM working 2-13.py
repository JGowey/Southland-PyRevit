...
Intent
------
Export selected Revit elements to a *fixed-column*, tab-delimited table (copied to
clipboard) for Excel PivotTables and downstream BOM / cut-sheet workflows.

This script unifies three source types into one export:
  1) Fabrication Parts (ITMs / FabricationPart)
  2) CP-driven RFAs (custom families using CP_* and CPL_* parameters)
  3) Native Revit MEP elements (pipes/ducts + fittings; also certain native family instances)
...
-----------------------------
* Sorting: numeric bucket to keep Excel pivots in a stable order (straights before fittings).
* Length_ft: decimal feet for totals. Length_ft_in: formatted feet/inches for cut sheets.
* Count vs length:
    - If a row has a measured length, Count is blank (so length drives totals).
    - If a row has no length, Count is 1 (so the row counts as one each).
* Item count: always 1 per exported row (used for counting instances).
* Area: pulled from CP_Construction Area (CP workflow tracking field), not a geometric area.
* Service: "system/service bucket" used for grouping (ITM service or native system labeling).
* ElementId: Revit built-in ElementId (el.Id.IntegerValue). Derived rows (e.g., All-Thread Rod)
  inherit the source element's ElementId for traceability.

Hangers / rods (Fabrication Hangers)
------------------------------------
Hangers can generate additional *derived* rows (e.g., All-Thread Rod) for BOM purposes.
Derived rows start as a copy of the parent hanger row and then override only the fields
that must differ (Category/Family/Description/Size/Length/etc.). Tracking columns like
Area, Service, Spool, and ElementId are intentionally inherited.

Notes
-----
- Comment/documentation pass: this file focuses on keeping comments accurate to behavior.
  Do not treat comments as a contract; the code is the source of truth.
"""

# -----------------------------------------------------------------------------
# Implementation notes
# -----------------------------------------------------------------------------
# - Read-only export: nothing is modified in the model.
# - Selection-driven: exports current selection, plus nested subcomponents for RFAs.
# - Fabrication Hangers: can emit additional derived rows (e.g., All-Thread Rod).
# - Pipe stock prefixing: Fabrication Pipework straights get 21FT-/40FT- prefix in Description.
# - Excel safety: size-like strings are emitted as formulas to avoid auto-date formatting.
# - Performance: caches type lookups and material name resolution in hot loops.
# -----------------------------------------------------------------------------
...
def get_cp_size_override(el):
    """Return CP_Size override text if populated (instance or type).
    CP_* params are Southland-authored; use for native Revit piping/fittings overrides where expected.
    """
    try:
        v = (get_param_str(el, 'CP_Size') or '').strip()
        if v:
            return v
    except Exception:
        pass
    try:
        ...
        if tid and tid.IntegerValue > 0:
            ...
            if t is not None:
                v = (get_param_str(t, 'CP_Size') or '').strip()
                if v:
                    return v
    except Exception:
        pass
...
def is_revit_mep_fitting(el):
    """True for native Revit pipe/duct fittings & accessories categories."""
    try:
...
def expand_selection_with_nested_rfas(doc, picked):
    """Split picked elements into non-RFA elements and FamilyInstances to process as RFAs.

    Nested-family discovery uses Revit's subcomponent graph:
      - Seed only selected FamilyInstances that actually have subcomponents (low overhead).
      - Recursively walk GetSubComponentIds to collect nested FamilyInstances.

    This avoids CP/CPL gating (which can miss nested families) and avoids expensive doc-wide scans.
    """
    non_rfa = []
    seed_rfa_ids = set()
    ...
    for el in picked:
        if isinstance(el, FamilyInstance):
            try:
                ...
                # Only seed if it actually has subcomponents
...
def is_pipework_pipe_itm(itm):
    """Return True when an ITM is a pipe *straight* in Fabrication Pipework.

    This drives 21FT-/40FT- stock prefixing. We intentionally gate on:
      - Fabrication Pipework category (string substring check), and
      - itm.IsAStraight() (tight definition of 'pipe stock' vs fittings)

    This avoids brittle ProductDataRange string formatting drift (e.g. 'Pipe (Straight)').
    """
    try:
...
def get_rfa_description(doc, fi):
    """
    RFA Description rules (per Southland NW VDC / Jacob):
      1) CP_Type Name (INSTANCE, then TYPE)  **but ignore if it matches CPL_Family Category**
      2) "{Family} - {Type}"

    Explicitly NOT used for Description:
      - CP_BOM_Description
      - CPL_Family Category / Classification
      - Revit Category name
    """

    def _cpl_family_category(_el):
        # Used ONLY to detect bad/auto-filled CP_Type Name values that mirror CPL.
        try:
            return get_param_str(_el, "CPL_Family Category").strip()
        except Exception:
            return ""

    def _cp_type_name(_el):
        try:
            return get_param_str(_el, CP_TYPE_NAME_PARAM).strip()
        except Exception:
            return ""

    # CPL value on instance/type (for comparison only)
    cpl_inst = _cpl_family_category(fi)
    te = None
    try:
        te = doc.GetElement(fi.GetTypeId())
    except Exception:
        te = None
    cpl_type = _cpl_family_category(te) if te else ""

    # 1) CP_Type Name on INSTANCE (ignore if it's just mirroring CPL)
    v = _cp_type_name(fi)
    if v and v != cpl_inst and v != cpl_type:
        return v

    # 1b) CP_Type Name on TYPE (ignore if it's just mirroring CPL)
    if te:
        v = _cp_type_name(te)
        if v and v != cpl_inst and v != cpl_type:
            return v

    # 2) Family + Type (should always exist for RFAs)
    # Prefer cached type info (more reliable than fi.Symbol in some categories)
    fam = ""
    typ = ""
    ...
    # Loud sentinel for QC (should never happen in valid content)
    return "MISSING_RFA_FAMILY_OR_TYPE"


def get_best_description_for_itm(doc, itm, fam, typ, base_desc):
    """
    FabricationPart Description rules (restored):
      1) CP_BOM_Description (instance)
      2) CP_Type Name (instance)
      3) Family - Type (when both are meaningful)
      4) base_desc (usually ProductLongDescription)
      5) Revit Category name or N/A
    """
    v = get_param_str(itm, CP_DESC_PARAM).strip()
    if v:
        return v

    v = get_param_str(itm, CP_TYPE_NAME_PARAM).strip()
    if v:
        return v
    ...
    if (base_desc or "").strip():
        return base_desc.strip()

    try:
        cn = itm.Category.Name if itm.Category else ""
        if cn:
            return cn
    except Exception:
        pass
...
def try_get_support_rod_usage(itm_part):
    """Return (rod_diameter_ft, rod_length_ft) for an ITM hanger.

    Native sources only:
      1) GetPartAncillaryUsage() items where UsageType == 'hanger' and Type == 'supportrod'
         - AncillaryWidthOrDiameter => diameter (ft)
         - Length => length (ft)  [often 0.0 in some content]

    If no supportrod ancillary usage exists, we optionally parse rod diameter from
    ITM-backed text fields *only when rod-specific tokens are present*.

    IMPORTANT:
      - Do NOT use BIMrx_* or CP_* parameters for rod size/length (reference only).
    """
    if not isinstance(itm_part, FabricationPart):
        return (None, None)

    # 1) Primary: ancillary usage object (fast, deterministic)
    try:
    ...
    for anc in anc_list:
        ...
        if u == "hanger" and t == "supportrod":
            ...
            if dia_ft is not None and abs(dia_ft) <= ZERO_TOL_FT:
                dia_ft = None
            if len_ft is not None and abs(len_ft) <= ZERO_TOL_FT:
                len_ft = None

            return (dia_ft, len_ft)

    # 2) Last resort: parse rod diameter from ITM-backed text (guarded by rod tokens)
...
def try_parse_itm_rod_diameter_ft(itm_part):
    ...
    Guardrails:
      - ONLY attempt parsing if the text contains rod tokens:
          'rod', 'all thread', 'all-thread', 'allthread'
      - Accept inch values in a plausible rod range: 0.10" to 1.00"
      - Prefer fractions (3/8, 1/2, 5/16) over decimals
    ...
    # NOTE:
    # Some single-rod hanger parts (e.g., clevis hangers) carry the rod diameter in the
    # part name/description (often as a decimal like 0.375) but DO NOT include "rod" tokens.
    # We allow parsing for hanger-category parts in that case, while keeping the strict
    # token guard for non-hangers to avoid false positives.
    is_hanger_cat = False
    try:
    ...
    # Fractions like 3/8, 1/2, 5/16
    # Only consider fractions when explicit rod tokens are present; this avoids
    # misreading common hanger/strut sizes like "1-5/8" as a rod diameter.
    if has_rod_token:
...
def build_itm_dimension_lookup(itm_part):
    """Build a {dimension_name: dimension_def} lookup for a FabricationPart.
    Passing this around avoids repeated GetDimensions() calls in hanger-heavy exports.
    """
    try:
...
def try_get_rod_lengths_ab_ft(itm_part, dim_lut=None):
    """Return (lenA_ft, lenB_ft) from FabricationPart dimensions.

    Source of truth (native-only):
      * "Length A"
      * "Length B"

    Notes:
      - Do NOT use 3rd-party instance parameters (BIMrx_*, CP_*, etc.).
      - Values are in internal feet.
      - Missing or near-zero values return None.
      - If dim_lut is provided, this function does not call GetDimensions().
    """
    if not isinstance(itm_part, FabricationPart):
        return (None, None)

    name_to_def = dim_lut or build_itm_dimension_lookup(itm_part)
...
def try_get_hanger_strut_length_ft(itm_part, dim_lut=None):
    """Return trapeze strut length (ft) for Fabrication hangers (native-only).

    Based on observed fabrication dimensions:
        Strut Length = Width + 2 * Bearer Extn

    Returns None if required dims are missing.
    If dim_lut is provided, this function does not call GetDimensions().
    """
    if not isinstance(itm_part, FabricationPart):
        return None
...
header = [
    "Sorting","Category","Family","Type","ItemNumber","Description","Size",
    "Length_ft","Length_ft_in","Count","Item count",
    "Model","Area","Material","Comments","Spool","Service","Hanger Number","Install Type","ElementId",
]
...
for el in non_rfa:
    ...
    revit_id_val = ""
    try:
        revit_id_val = str(el.Id.IntegerValue)
    except Exception:
        revit_id_val = ""
    ...
    material_val = ""
    service_val = ""
    install_type_val = ""  # set per-branch (ITM/native curve/etc.)
    model_val = "N/A"
    ...
    if isinstance(el, FabricationPart):
        ...
        # Description (restored behavior):
        # 1) CP_BOM_Description, 2) CP_Type Name, 3) Family - Type, 4) ProductLongDescription
        # Then apply pipe stock prefixing for Fabrication Pipework straights.
        base_desc = (el.ProductLongDescription or "")
        desc_seed = get_best_description_for_itm(doc, el, fam, typ, base_desc)
        desc_val = apply_pipe_prefix_to_description(el, desc_seed, length_ft)

        # Fabrication ductwork: Description (col F) should be the fabrication family name only.
        ...
        if is_hanger:
            # Build fabrication dimension lookup ONCE per hanger for rod + strut calculations
            dim_lut = build_itm_dimension_lookup(el)
            lenA_ft, lenB_ft = try_get_rod_lengths_ab_ft(el, dim_lut)
            # Two-rod hanger if Length B is present and non-zero (Length B can exist but be 0.0 on 1-rod)
            is_two_rod = (lenB_ft is not None) and (abs(lenB_ft) > ZERO_TOL_FT)
            if is_two_rod:
                # Trapeze: use strut length on the hanger row (requested for BOM)
                strut_len_ft = try_get_hanger_strut_length_ft(el, dim_lut)
        ...
        length_for_count = length_ft
        size_val = get_cp_size_override(el) or get_revit_curve_size_text(el)
    ...
    elif is_revit_mep_fitting(el):
        # Native Revit pipe/duct fittings & accessories.
        # If CP_Size is populated (Southland override), prefer it; otherwise use native calculated size.
        _sz = get_cp_size_override(el) or get_native_revit_size_text(el)
        if _sz:
            size_val = _sz
        material_val = get_native_revit_material(doc, el)
        service_val = get_native_revit_service(el)
        install_type_val = get_native_revit_install_type(el)
        # Description for native fittings: use Family (and Type) so it is identifiable
        _t = (typ or '').strip()
        _f = (fam or '').strip()
        if _t and _t != 'N/A':
            desc_val = '{} - {}'.format(_f or (get_revit_category_name(el) or 'MEP'), _t)
        else:
            desc_val = _f or (get_revit_category_name(el) or 'MEP')

    elif isinstance(el, FamilyInstance):
        # Generic RFAs / non-MEP family instances (e.g., Structural Connections, Framing, Accessories)
        # Description rules: CP_Type Name (instance/type) then Family - Type
        desc_val = get_rfa_description(doc, el)

        # Size: prefer CP_Size if present, else native family size text, else N/A
        _cp_size = get_param_str(el, CP_SIZE_PARAM).strip()
        if _cp_size:
            size_val = excel_formula(format_size(_cp_size))
        else:
            size_val = get_native_revit_size_text(el) or size_val

        # Service/material/install type: use native helpers when applicable
        if not service_val:
            service_val = get_native_revit_service(el)
        if not install_type_val:
            install_type_val = get_native_revit_install_type(el)
        if not material_val:
            material_val = get_native_revit_material(doc, el) or material_val
    ...
    # Built-in Revit ElementId (unique per element in the model)
    try:
        revit_id_val = str(el.Id.IntegerValue)
    except Exception:
        revit_id_val = ""
    ...
    if isinstance(el, FabricationPart) and is_hanger:
        # Rod rows (native-only):
        # - Prefer fabrication dimensions Length A / Length B when present
        # - Output Rod B ONLY when non-zero (Length B exists but can be 0.0 on 1-rod hangers)
        if (lenA_ft is not None) and (abs(lenA_ft) > ZERO_TOL_FT):
            rod_row_1 = build_rod_row_from_hanger_cached(base_row, dia_ft, lenA_ft, el)
            if rod_row_1:
                rows_append(rod_row_1)

            if (is_two_rod
                and (lenB_ft is not None)
                and (abs(lenB_ft) > ZERO_TOL_FT)):
                rod_row_2 = build_rod_row_from_hanger_cached(base_row, dia_ft, lenB_ft, el)
                if rod_row_2:
                    rows_append(rod_row_2)
        else:
            # Fallback: use ancillary-derived rod length (may still be None)
...
for fi in rfas_expanded:
    ...
    if not (model_val or "").strip():
        ...
        # Built-in Revit ElementId for this family instance
    try:
        fi_id_val = str(fi.Id.IntegerValue)
    except Exception:
        fi_id_val = ""
...
for _r in rows:
    try:
        _r[IDX_TYP] = normalize_for_pivot(_r[IDX_TYP])
