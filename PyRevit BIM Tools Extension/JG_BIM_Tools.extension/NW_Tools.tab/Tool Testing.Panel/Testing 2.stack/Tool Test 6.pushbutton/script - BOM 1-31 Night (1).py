...
def is_native_familyinstance(fi):
    """True for native Revit fittings being treated as RFAs (FamilyInstance) but NOT part of CP/CPL workflow.
    Gate is intentionally strict to avoid drift impacting true RFAs.
    """
    try:
        if not isinstance(fi, FamilyInstance):
            return False
        # CPL_Family Category present => your RFA workflow
        if get_param_str(fi, CPL_FAMILY_CATEGORY_PARAM).strip():
            return False
        try:
            sym = fi.Symbol
        except Exception:
            sym = None
        if sym and get_param_str(sym, CPL_FAMILY_CATEGORY_PARAM).strip():
            return False
        # CP fields present => intentional RFA
        if get_param_str(fi, CP_SIZE_PARAM).strip() or get_param_str(fi, CP_DESC_PARAM).strip() or get_param_str(fi, CP_TYPE_NAME_PARAM).strip():
            return False
        return True
    except Exception:
        return False
