...
def get_rfa_description(doc, fi):
    ...
    try:
        ...
        if te:
            try:
                return (Element.Name.GetValue(te) or "").strip()
            except:
...
def get_category_for_export(doc, el):
    ...
    try:
        ...
        te = None
    if te:
        s2 = get_param_str(te, CPL_FAMILY_CATEGORY_PARAM).strip()
        if s2:
            return s2
    return get_revit_category_name(el)
...
def get_comments(doc, el):
    ...
    try:
        te = doc.GetElement(el.GetTypeId())
        if te:
            s2 = get_bip_str(te, BuiltInParameter.ALL_MODEL_TYPE_COMMENTS)
            if s2:
                return s2
    except:
        pass
...
def get_family_type(doc, el):
    ...
    Fixes RFA type pulling using Element.Name.GetValue(typeElement).
    """
    if isinstance(el, FamilyInstance):
        fam = ""
        typ = ""
        ...
        if te:
            try:
                typ = (Element.Name.GetValue(te) or "").strip()
            except:
                typ = ""
            if not typ:
                try:
                    typ = (te.Name or "").strip()
                except:
                    typ = ""
        if not typ:
            try:
        ...
        if te:
            ...
            if not typ:
                try:
                    ...
                    typ = ""
            try:
                fam = (getattr(te, "FamilyName", None) or "").strip()
            except:
                fam = ""
            if not fam:
                try:
                    fam = get_bip_str(te, BuiltInParameter.ALL_MODEL_FAMILY_NAME)
                except:
                    fam = ""
        if not typ:
            try:
...
def get_model_field_for_rfa(doc, fi):
    ...
    try:
        ...
        te = None
    if te:
        s2 = get_bip_str_by_id(te, BIP_TYPE_MARK_ID_GUESS)
        if s2:
            return s2
        try:
            tn = (Element.Name.GetValue(te) or "").strip()
            if tn:
                return tn
        except:
            pass
...
def is_itm_hanger(itm):
    if not isinstance(itm, FabricationPart):
        return False
    dia_ft, len_ft = try_get_support_rod_usage(itm)
...
def build_rod_row_from_hanger(doc, hanger_row, itm_hanger):
    dia_ft, rod_len_ft = try_get_support_rod_usage(itm_hanger)
...
for el in non_rfa:
    ...
    if isinstance(el, FabricationPart):
        ...
        size_val = excel_text_formula(format_size_inch_marks(base_size))
    ...
    base_row = [
        ...
        fmt_len_decimal_ft(length_ft), fmt_len_ft_in_frac(length_ft, denom=FRAC_DENOM),
        count_flag(length_for_count),
        "1",  # Item count
    ...
    if isinstance(el, FabricationPart) and is_itm_hanger(el):
        rod_row = build_rod_row_from_hanger(doc, base_row, el)
        if rod_row:
            rows.append(rod_row)
...
for fi in rfas_expanded:
    ...
    size_val = excel_text_formula(format_size_inch_marks(get_param_str(fi, CP_SIZE_PARAM)))
