...
def get_revit_curve_size_text(el):
    """Best-effort size extraction for native Revit MEP curves.

    Returns an Excel-safe text formula (already wrapped) or "".
    """
    try:
        cid = el.Category.Id.IntegerValue if (el and el.Category and el.Category.Id) else None
    except Exception:
    ...
    # Pipes / Flex Pipes: diameter
    if cid in (int(BuiltInCategory.OST_PipeCurves), int(BuiltInCategory.OST_FlexPipeCurves)):
        p = get_bip_param(el, BuiltInParameter.RBS_PIPE_DIAMETER_PARAM)
        if not p:
            p = get_bip_param(el, BuiltInParameter.RBS_CURVE_DIAMETER_PARAM)
        try:
            dia_ft = float(p.AsDouble()) if p else None
        except Exception:
            dia_ft = None
        if dia_ft is None:
            return ""
        return excel_text_formula(fmt_inches_frac(_ft_to_in(dia_ft), denom=FRAC_DENOM))

    # Ducts / Flex Ducts: diameter OR width x height
    if cid in (int(BuiltInCategory.OST_DuctCurves), int(BuiltInCategory.OST_FlexDuctCurves)):
        p_dia = get_bip_param(el, BuiltInParameter.RBS_CURVE_DIAMETER_PARAM)
        try:
            dia_ft = float(p_dia.AsDouble()) if p_dia else None
        except Exception:
            dia_ft = None
        if dia_ft and dia_ft > ZERO_TOL_FT:
            return excel_text_formula(fmt_inches_frac(_ft_to_in(dia_ft), denom=FRAC_DENOM))

        p_w = get_bip_param(el, BuiltInParameter.RBS_CURVE_WIDTH_PARAM)
        p_h = get_bip_param(el, BuiltInParameter.RBS_CURVE_HEIGHT_PARAM)
        try:
            w_ft = float(p_w.AsDouble()) if p_w else None
        except Exception:
            w_ft = None
        try:
            h_ft = float(p_h.AsDouble()) if p_h else None
        except Exception:
            h_ft = None
        if w_ft and h_ft:
            w_in = _ft_to_in(w_ft)
            h_in = _ft_to_in(h_ft)
            return excel_text_formula(
                '{} x {}'.format(fmt_inches_frac(w_in, denom=FRAC_DENOM), fmt_inches_frac(h_in, denom=FRAC_DENOM)).replace('"', '"')
            )
    return ""
...
def normalize_for_pivot(val, placeholder):
...
def try_get_rod_lengths_ab_ft(itm_part):
    """Returns (lenA_ft, lenB_ft) for fabrication hangers from the fabrication database (vendor-independent).

    Primary source: FabricationPart.GetDimensions() + GetDimensionValue(FabricationDimensionDefinition)
      - Length A
      - Length B
      - Fallback for B: Drop Rod Length #2

    Secondary (last-resort) source: Revit parameters (in case a particular hanger doesn't expose dims).
    """
    ...
    # --- 1) Fabrication dimensions (source of truth) ---
    try:
        ...
        def _get_dim_ft(name_lower):
            ...
            try:
                ...
                v = None
            # Some dims may be calculated; try calculated value if direct is None
            if v is None:
                try:
        ...
        la = _get_dim_ft("length a")
        lb = _get_dim_ft("length b")

        # Some hanger definitions use a special second-drop dim name; only use if Length B missing
        if lb is None:
            lb = _get_dim_ft("drop rod length #2")

        # normalize -0.0, etc.
        if la is not None and abs(la) < ZERO_TOL_FT:
            la = None
        if lb is not None and abs(lb) < ZERO_TOL_FT:
            lb = None
    except Exception:
        pass

    # --- 2) Secondary fallback: parameters (keeps script resilient if dims aren't exposed) ---
    if la is None or lb is None:
        def _get_len_by_names(names):
            # 1) direct double
            for nm in names:
                v = get_param_double(itm_part, nm)
                if v is not None and abs(v) > ZERO_TOL_FT:
                    return v
            # 2) value string parse
            for nm in names:
                vs = get_param_str(itm_part, nm)
                v = parse_len_value_to_ft(vs)
                if v is not None and abs(v) > ZERO_TOL_FT:
                    return v
            return None

        namesA = ["Length A","Rod Length A","RodLength A","Rod A","All Thread Length A","All-Thread Length A"]
        namesB = ["Length B","Rod Length B","RodLength B","Rod B","All Thread Length B","All-Thread Length B"]

        if la is None:
            la = _get_len_by_names(namesA)
        if lb is None:
            lb = _get_len_by_names(namesB)

        # scan all parameters for best match (handles odd spacing/casing)
        if la is None or lb is None:
            try:
                for p in itm_part.Parameters:
                    try:
                    ...
                    if not nm:
                        continue
                    # pull value string safely
                    try:
                        vs = (p.AsValueString() or "").strip()
                    except Exception:
                        try:
                            vs = (p.AsString() or "").strip()
                        except Exception:
                            vs = ""
                    # pull double safely
                    dv = None
                    ...
                    if la is None and (nm == "length a" or nm.endswith("length a")):
                        la = dv if (dv is not None and abs(dv) > ZERO_TOL_FT) else parse_len_value_to_ft(vs)
                    if lb is None and (nm == "length b" or nm.endswith("length b")):
                        lb = dv if (dv is not None and abs(dv) > ZERO_TOL_FT) else parse_len_value_to_ft(vs)
