...
def get_model_field_for_rfa(doc, fi):
    """
    RFA Model priority (per Southland workflow):
      1) CP_Model Number (instance if present; if it's a Type param it may still be readable via LookupParameter)
      2) Revit built-in "Model" (Identity Data -> Model)
      3) N/A
    Notes:
      - Treat empty, '-', and 'N/A' (any case) as blank.
      - Do NOT fall back to Type Name (that was causing false 'model' values like PL-4-...).
    """
    ...
    # 2) Revit Identity Data -> Model
    s2 = get_param_str(fi, "Model")
