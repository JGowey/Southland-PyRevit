...
Modeless UI:
  ...
  - Scope radios: Current View (default), Entire Model, Current Selection
  - Reusable Run button (select/run/select/run)
  - Larger window + scrollable report

Performance:
  - Category-name based filter (matches your list)
  - Chunked Transactions for large models
  - No display-unit conversions and no unit detection
"""
...
# Fabrication API class (safe import)
try:
...
# Collection
# -----------------------------------------------------------------------------
def collect_elements(doc_, uidoc_, scope_str, cat_filter):
    def keep(e):
        try:
            if is_fab_part(e):
                return True
    ...
    if scope_str == "Current Selection":
        ...
        return [e for e in els if e is not None and keep(e)]
    ...
    return [e for e in collector if keep(e)]
    ...
    def Execute(self, uiapp):
        try:
            ...
            if not (self._req.do_weight or self._req.do_volume or self._req.do_bomcat):
                lines.append("")
                lines.append("Nothing selected to run (check at least one box).")
                self._set_status(lines)
                return
            ...
            lines.append("Transaction: completed")
            if self._req.do_weight:
            ...
            logger.exception("ExternalEvent Execute failed.")
            self._set_status(["Run failed.", "", "ERROR:", str(ex)])

    def _set_status(self, lines):
        try:
            self._win.StatusText.Text = "\n".join(lines)
        except:
            pass

# -----------------------------------------------------------------------------
# WPF Window (bigger + scroll)
# -----------------------------------------------------------------------------
XAML = u"""
    ...
    <Border Grid.Row="4" BorderBrush="#CFCFCF" BorderThickness="1" CornerRadius="6" Padding="10">
      <ScrollViewer VerticalScrollBarVisibility="Auto" HorizontalScrollBarVisibility="Disabled">
        <TextBlock x:Name="StatusText"
                   Text="Ready. Change selection/view then click Run as many times as you want."
                   TextWrapping="Wrap"/>
      </ScrollViewer>
    </Border>
...
class BatchWindow(object):
    ...
    def _on_run(self, sender, args):
        try:
            ...
            self.StatusText.Text = "Queued... Revit will execute the run shortly."
            self._ext_event.Raise()
        except Exception as ex:
            self.StatusText.Text = "UI error: {}".format(ex)
