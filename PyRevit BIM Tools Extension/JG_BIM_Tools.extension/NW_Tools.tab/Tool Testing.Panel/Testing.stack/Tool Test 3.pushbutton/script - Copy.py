...
def rfa_group_key(fi):
    """
    UPDATED: group by Family + *TypeId* (hard discriminator) + the CP descriptors.
    This prevents different Revit Types that share a name from being lumped together.
    """
    return (
        rfa_family_name(fi),
        rfa_type_id_int(fi),        # <-- critical fix
        rfa_type_name(fi),          # kept for readability/debug
...
def sort_key_rfa(fi):
    ...
    1) Revit Category
    2) Family
    3) Type (by Type Name, then TypeId)
    4) CP_Length (numeric if possible; otherwise string)
    5) ElementId (tie-breaker)
    """
    ...
    cp_len_str = get_param_str(fi, "CP_Length")
    try:
        cp_len = float(cp_len_str)
    except:
        cp_len = cp_len_str

    return (cat, fam, typ, tid, cp_len, fi.Id.IntegerValue)
...
# ---------------- Grouping / numbering (OVERWRITE, no blank checks) ----------------
...
forms.alert(
    ...
    "RFA group: Family + TypeId (+ CP descriptors)\n"
