...
class _WorkState(object):
    ...
    def _prepare_scope(self):
        ...
        if mode == u'selection':
            ...
            # Retry once if we suspiciously got 0 candidates on first run
            try:
                if not ids:
                    fec2 = DB.FilteredElementCollector(doc, doc.ActiveView.Id)
                    fec2 = fec2.WhereElementIsNotElementType().WherePasses(compile_union_filter(self.rules))
                    ids = list(fec2.ToElementIds())
            except:
                pass
        else:
    ...
    def _apply_batch(self):
        ...
        try:
            ...
            for i in range(self.write_idx, hi):
                ...
                # Early skip: if incoming is string and equals current text, skip without set
                try:
                    _string_types = (unicode, str)
                except:
                    _string_types = (str,)
                if isinstance(upd.get('value'), _string_types):
                    _incoming_txt = safe_str(upd.get('value')).strip()
                    if safe_str(before).strip() == _incoming_txt:
                        self.counts['skips'] += 1
                        continue
