...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        ...
        Title="Model Setup"
        Width="750" Height="760" MinHeight="720"
...
<GroupBox.Header>
        ...
        <StackPanel>
          <CheckBox x:Name="ChkParams"
                    Content="Install/Bind CP__BOM__Category, CP__Weight, CP__Volume (Construction)"
                    Margin="0,2,0,6"/>
          <TextBlock Text="Load into RFAs (Family Parameters):" Margin="18,0,0,4"/>
          <CheckBox x:Name="ChkRfaWeight"
                    Content="CP__Weight → Construction"
                    Margin="36,0,0,2"/>
          <CheckBox x:Name="ChkRfaVolume"
                    Content="CP__Volume → Construction"
                    Margin="36,0,0,2"/>
          <CheckBox x:Name="ChkRfaWpf"
                    Content="CP__Weight__Per__Foot → Set"
                    Margin="36,0,0,10"/>

          <TextBlock Text="Note: To load parameters into RFAs, you must be in the Family Editor (FamilyManager)."
                     Margin="36,0,0,10" Opacity="0.75"/>
...
class ModelSetupWindow(object):
    def __init__(self):
        ...
        self.ChkRfaWeight = self._w.FindName("ChkRfaWeight")
        self.ChkRfaVolume = self._w.FindName("ChkRfaVolume")
        self.ChkRfaWpf = self._w.FindName("ChkRfaWpf")
...
Installs/binds the shared parameters the tool writes to (project parameter binding).

- CP_Weight: Stores calculated/derived dry weight.
- CP_Volume: Stores calculated/derived volume.
- CP_BOM_Category: Stores a simplified category label for BOM/QC use.

Load into RFAs (Family Parameters):
- Adds shared parameters as FAMILY parameters when you are in the Family Editor (FamilyManager).
- CP_Weight_Per_Foot is a unit weight per foot used for linear families:
  CP_Weight_Per_Foot × Length → CP_Weight
...
Buttons:
- Run Setup: Executes the selected schedule imports and/or parameter installation.
- Back: Returns to the main Weight Tool window.
- Close: Exits Model Setup without running.
...
Tip: If something fails, copy the Status text and send it to BIM/VDC.""",
    ...
    def _run(self, sender, args):
        ...
        _REQ.rfa_add_cp_weight = bool(self.ChkRfaWeight.IsChecked)
        _REQ.rfa_add_cp_volume = bool(self.ChkRfaVolume.IsChecked)
        _REQ.rfa_add_cp_wpf = bool(self.ChkRfaWpf.IsChecked)
