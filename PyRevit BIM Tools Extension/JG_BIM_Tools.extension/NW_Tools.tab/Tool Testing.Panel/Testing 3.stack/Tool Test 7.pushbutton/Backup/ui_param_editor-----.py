...
GROUP_KEY_CHOICES = [
    ...
    "Structural Analysis", "Structural Section Dimensions", "Sub-Division", "Visualization", "Text", "Title Text", "Visibility"
]
...
class ParamEditor(Form):
    def __init__(self, top_spf, row):
        ...
        for g in GROUP_KEY_CHOICES: self.cboGroup.Items.Add(g)
    ...
    def _apply_row(self, row):
        ...
        try:
            ...
            for i in range(self.cboGroup.Items.Count):
                if self.cboGroup.Items[i].lower() == (row.group_key or "Construction").lower():
                    idx = i; break
