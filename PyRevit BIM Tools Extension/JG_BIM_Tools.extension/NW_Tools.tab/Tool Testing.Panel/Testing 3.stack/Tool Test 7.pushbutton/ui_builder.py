...
class BuilderForm(Form):
    def __init__(self, json_path=None):
        ...
        btnTest = Button(Text="Test"); btnTest.Location = Point(632, 650); btnTest.Size = Size(100, 28)
        btnTest.Anchor = AnchorStyles.Bottom | AnchorStyles.Left; btnTest.Click += self._on_test
        ...
        for w in [lblDN, self.txtDN, lblScope, self.cboScope, lblTop, self.txtTop, btnBrowseTop, lblNote,
                  self.lv, btnAdd, btnEdit, btnDup, btnRemove, btnClear, btnTest, btnSave, btnCancel]:
            self.Controls.Add(w)
    ...
    def _on_test(self, s, a):
        """Dry-run validation of the current list against the active document."""
        try:
            disp = (self.txtDN.Text or "").strip() or "(unsaved config)"
            js = {"display_name": disp, "parameters": [r.to_dict() for r in self.rows]}
            top_sp = (self.txtTop.Text or "").strip()
            if top_sp:
