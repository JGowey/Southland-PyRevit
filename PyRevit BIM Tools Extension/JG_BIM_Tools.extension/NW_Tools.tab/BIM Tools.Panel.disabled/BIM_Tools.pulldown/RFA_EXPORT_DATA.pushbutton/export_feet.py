...
for pname in sorted_param_names:
    ...
    if hasattr(pdef, "ParameterType") and pdef.ParameterType == ParameterType.Length:
        header.append("{0} (Feet)".format(pname))
    else:
        header.append(pname)
        ...
        if p and p.HasValue:
            try:
                if p.Definition.ParameterType == ParameterType.Length:
                    val = str(round(p.AsDouble(), 6))  # Native Revit units = decimal feet
                else:
...
save_dialog.FileName = "FamilyType_Export.csv"
