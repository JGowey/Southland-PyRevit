...
# Supported data types (must match column_settings.py)
EXTRA_COL_TYPES = [
    "text", "number", "integer", "boolean",
    "length", "weight / mass", "area", "volume",
    "flow rate", "pressure", "temperature", "angle", "velocity", "force",
]

# Revit internal -> display unit conversion factors.
# None = handled by special-case code in read_extra_param_value().
...
_UNIT_FACTORS = {
    "ft_in_frac":  None,      # special formatter
    "ft_decimal":  1.0,
    "in_decimal":  12.0,
    "mm":          304.8,
    "cm":          30.48,
    ...
    "inwc":        0.192221,
    "inhg":        0.014139,
    "degF":        None,      # direct value -- no conversion
    "degC":        None,      # (F - 32) * 5/9
...
# Temperature types that need special-case conversion
_TEMP_UNIT_KEYS = {"degF", "degC", "kelvin", "rankine"}


class ExtraColumnConfig(object):
    """Represents one user-defined extra export column."""

    def __init__(self, param_name="", display_name="", data_type="text",
                 unit_key=None):
        self.param_name   = param_name    # Revit parameter name to read
        self.display_name = display_name  # TSV column header
        self.data_type    = data_type     # one of EXTRA_COL_TYPES
        self.unit_key     = unit_key      # one of _UNIT_FACTORS keys, or None
...
def _active_profile_path():
    """Return path to the active-profile tracker JSON."""
    appdata = os.environ.get("APPDATA", os.path.expanduser("~"))
    return os.path.join(appdata, "pyRevit", "NW_BOM", "active_profile.json")


def _profiles_dir():
    """Return the profiles directory (does not create it)."""
    appdata = os.environ.get("APPDATA", os.path.expanduser("~"))
    return os.path.join(appdata, "pyRevit", "NW_BOM", "profiles")
...
def _read_json_file(path):
    """Read JSON safely under IronPython 2.7 (explicit UTF-8, binary open)."""
...
def _write_json_file(path, data):
    """Write JSON with ensure_ascii=True so all chars are safely escaped."""
    text = json.dumps(data, indent=2, ensure_ascii=True)
    with open(path, "wb") as fh:
        fh.write(text.encode("utf-8"))


def load_extra_columns(path=None):
    """Load extra column configs for the currently active profile.

    Returns list[ExtraColumnConfig].  Returns [] if no profile is active
    or if the profile file cannot be read.
    """
    # path argument kept for backwards compat but ignored -- we always use
    # the active profile from the profile manager.
    active_path = _active_profile_path()
    if not os.path.isfile(active_path):
        return []
    try:
        data        = _read_json_file(active_path)
        active_name = data.get("active") or None
    except Exception:
        return []
    ...
    profile_file = _profile_path(active_name)
    if not os.path.isfile(profile_file):
        return []
    try:
        pdata = _read_json_file(profile_file)
        return [ExtraColumnConfig.from_dict(d) for d in pdata.get("extra_columns", [])]
    except Exception:
        return []


def save_extra_columns(configs, path=None):
    """No-op shim kept for backwards compat. Profiles are saved by column_settings.py."""
    pass


def read_extra_param_value(element, col_config, context):
    """Read one extra-column value from a Revit element.

    Returns a formatted string ready for TSV, or "" if not found.
    Derived rows (no real source element) always return "".
    """
    if element is None:
        return ""

    ph = ParameterHelper(context)
    p  = ph.get_param(element, col_config.param_name)

    # Fall back to type element if not on instance
    if p is None:
        try:
            te = ph.get_type_element(element)
            if te:
                p = ph.get_param(te, col_config.param_name)
        except Exception:
            p = None
    ...
    dtype = col_config.data_type
    ukey  = col_config.unit_key
    ...
    try:
        ...
        # ---- numeric types with unit conversion ------------------------
        elif dtype in ("length", "weight / mass", "area", "volume",
                       "flow rate", "pressure", "angle", "velocity", "force"):
            try:
            ...
            if ukey == "ft_in_frac":
                return context.formatters.length_fractional.format(v_internal)

            factor = _UNIT_FACTORS.get(ukey, 1.0)
            if factor is None:
                factor = 1.0
            return "{:.4g}".format(v_internal * factor)

        # ---- temperature (special conversion) --------------------------
        elif dtype == "temperature":
            try:
                v_f = p.AsDouble()   # Revit internal = Fahrenheit
            except Exception:
                return (p.AsValueString() or "").strip()

            if ukey == "degF" or ukey is None:
                return "{:.2f}".format(v_f)
            elif ukey == "degC":
                return "{:.2f}".format((v_f - 32.0) * 5.0 / 9.0)
            elif ukey == "kelvin":
                return "{:.2f}".format((v_f - 32.0) * 5.0 / 9.0 + 273.15)
            elif ukey == "rankine":
                return "{:.2f}".format(v_f + 459.67)
            else:
                return "{:.2f}".format(v_f)
...
class BOMRow:
    ...
    def __init__(self):
        ...
        # Extra user-configured columns (populated in _stage_extra_columns)
        self.extra_values = []  # list[str], parallel to ExportContext.extra_columns
    ...
    def to_list(self):
        ...
        return base + list(self.extra_values)
    ...
    def copy(self):
        ...
        new_row.extra_values = list(self.extra_values)
        return new_row
...
class ExportContext:
    ...
    def __init__(self, doc):
        ...
        # Extra user-configured columns loaded from JSON at startup
        self.extra_columns = load_extra_columns()  # list[ExtraColumnConfig]
    ...
    def reset(self):
        ...
        self.cache.clear()
        self.extra_columns = load_extra_columns()
...
class SizeExtractor(FieldExtractor):
    ...
    def extract_itm(self, element):
        ...
        # 3. Format with inch marks and return as plain text
        return self.context.formatters.size.format(base_size)
    ...
    def extract_native_curve(self, element):
        ...
        if override:
            return self.context.formatters.size.format(override)
    ...
    def extract_rfa(self, element):
        ...
        if cp_size:
            return self.context.formatters.size.format(cp_size)
    ...
    def _get_native_size(self, element):
        ...
        if not s:
            return ""
        s = s.replace("/", " x ")
        return self.context.formatters.size.format(s)
...
class HangerProcessor:
    ...
    def _build_rod_row(self, hanger_row, dia_ft, rod_len_ft, itm_hanger):
        ...
        if dia_ft is not None:
            dia_in = dia_ft * 12.0
            rod_row.size = self.context.formatters.inches_fractional.format(dia_in)
        else:
            rod_row.size = "N/A"
...
class BOMExportPipeline:
    ...
    def execute(self, elements):
        ...
        # Stage 4b: Populate extra user-defined columns
        rows = self._stage_extra_columns(rows)
        log_debug("Extra columns populated", "EXTRA_COLS")
    ...
    def _stage_extra_columns(self, rows):
        """Stage 4b: Read user-defined extra columns from each row's source element.

        Derived rows (synthesised rod rows) get blank values for all extra columns,
        since there is no real Revit element to read from.
        """
        extra_cols = self.context.extra_columns
        if not extra_cols:
            return rows

        for row in rows:
            if row._is_derived or row._source_element is None:
                row.extra_values = [""] * len(extra_cols)
            else:
                row.extra_values = [
                    read_extra_param_value(row._source_element, col, self.context)
    ...
    def _stage_serialize(self, rows):
        ...
        # Build header -- base columns plus any extra column display names
        extra_headers = [col.display_name for col in self.context.extra_columns]
        header = self.config.Columns.HEADER + extra_headers
...
def _validate_extra_columns(extra_columns, elements, doc):
    """Check that each extra column param name resolves on at least one element.

    Returns a list of param names that were found on zero elements.
    Samples up to 30 elements for speed -- enough to catch genuine typos.
    """
    if not extra_columns or not elements:
        return []
    ...
    for col in extra_columns:
        ...
        for el in sample:
            try:
                p = el.LookupParameter(col.param_name)
                if p is not None:
                    found = True
                    break
            except Exception:
            ...
            # Also check type element
            if not found:
                try:
                    te = doc.GetElement(el.GetTypeId())
                    if te:
                        p = te.LookupParameter(col.param_name)
                        if p is not None:
                            found = True
                            break
                except Exception:
...
def main():
    ...
    # Create context (loads active extra column profile)
    ...
    # Validate extra column parameter names against the selection
    if context.extra_columns:
        missing_params = _validate_extra_columns(context.extra_columns, elements, doc)
        if missing_params:
            msg = (
                "The following extra column parameter(s) were not found on any "
                "selected element:\n\n"
                "{}\n\n"
                "These columns will export as blank. This may mean the parameter "
                "name is misspelled or doesn't apply to the selected element types.\n\n"
                "Continue anyway?"
            ).format("\n".join("  - " + p for p in missing_params))
            if not forms.alert(msg, title="Extra Column Warning", yes=True, no=True):
...
def show_completion_dialog(row_count):
    ...
    # Show which extra column profile was active (if any)
    try:
        active_profile = get_active_profile()
        if active_profile:
            profile_line = "Extra columns from profile:  {}".format(active_profile)
        else:
            profile_line = "No extra column profile active (standard columns only)"
    except Exception:
        profile_line = ""
    ...
    if profile_line:
        ...
        components.append(Label(profile_line))

    components += [
        Label(""),
        Label("NEXT STEP:"),
        Label("1. Open the NW__BOM__Template V2 Excel file"),
...
def download_template(sender, args):
    ...
    try:
        ...
        # Template filename
        template_name = "NW_BOM_Template V2.xlsm"
