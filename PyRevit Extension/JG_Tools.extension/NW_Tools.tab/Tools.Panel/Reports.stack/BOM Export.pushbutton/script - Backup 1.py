...
class ElementClassifier:
    ...
    def is_revit_mep_fitting(self, element):
        """Check if element is a native Revit MEP fitting."""
        try:
...
class LengthExtractor(FieldExtractor):
    ...
    def extract_itm(self, element):
        ...
        # Fishmouth override
        try:
            if element.ItemCustomId == self.config.Rules.FISHMOUTH_CID:
...
class FabricationPartProcessor(ElementProcessor):
    ...
    def process(self, element):
        ...
        desc_seed = self.extractors.description.extract_itm(element, row)
        row.description = self._apply_pipe_prefix(element, desc_seed, length_ft)
    ...
    def _apply_pipe_prefix(self, element, base_desc, length_ft):
        """Apply 21FT-/40FT- prefix to pipe straights."""
        if not base_desc:
            base_desc = ""
        
        if not self._is_pipework_pipe(element):
            return base_desc
        
        # Already has prefix?
        if base_desc.startswith(self.config.Format.PIPE_PREFIX_SHORT) or \
           base_desc.startswith(self.config.Format.PIPE_PREFIX_LONG):
            return base_desc
        
        # Determine prefix
        psd = self._get_product_short_description(element).lower()
        nums = re.findall(r"\d+", psd)
        nums = [int(n) for n in nums] if nums else []
        
        if 40 in nums:
            prefix = self.config.Format.PIPE_PREFIX_LONG
        elif 21 in nums or 20 in nums:
            prefix = self.config.Format.PIPE_PREFIX_SHORT
        else:
            try:
                lf = float(length_ft) if length_ft is not None else 0.0
            except:
                lf = 0.0
            prefix = self.config.Format.PIPE_PREFIX_SHORT if lf <= self.config.Rules.PIPE_STOCK_THRESHOLD_FT else self.config.Format.PIPE_PREFIX_LONG
    ...
    def _get_product_short_description(self, itm):
        """Get ITM product short description."""
        try:
        ...
        for nm in ("Product Short Description", "ProductShortDescription"):
            s = param_helper.get_param_str(itm, nm).strip()
            if s:
                return s
        return ""
...
class NativeMEPFittingProcessor(ElementProcessor):
    """Processor for native Revit MEP fittings."""
