...
class SizeExtractor(FieldExtractor):
    ...
    def extract_itm(self, element):
        ...
        ITMs use the native Revit fabrication API property (ProductSizeDescription)
        as the authoritative size source. CP_Size is NOT checked here — that is
        reserved for RFAs. Ductwork falls back to OverallSize parameters if
        ProductSizeDescription is empty.
        """
        # 1. ProductSizeDescription — native fabrication API, authoritative for ITMs
        base_size = (element.ProductSizeDescription or "").strip()
        
        # 2. Ductwork fallback (OverallSize) — only when ProductSizeDescription is blank
