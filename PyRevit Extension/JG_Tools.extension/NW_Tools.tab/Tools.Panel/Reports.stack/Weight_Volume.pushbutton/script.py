...
class BatchWriteHandler(IExternalEventHandler):
    ...
    def Execute(self, uiapp):
        ...
        try:
            ...
            # ------------------------
            if getattr(self._req, "report_only", False):
                sum_w_kg = 0.0
                sum_v_ft3 = 0.0
                ...
                for el in elements:
                    # -------- CP_Weight --------
                    pW = el.LookupParameter("CP_Weight")
                    if pW is not None:
                        # Parameter exists; check if value is NULL (unset)
                        try:
                            has_w = pW.HasValue
                        except Exception:
                            has_w = True
                        ...
                        if not has_w:
                            ...
                            try:
                                wf = float(pW.AsDouble())
                            except Exception:
                                # If it can't be read as a number, treat as NULL
                                null_weight_count += 1
                            else:
                                if wf == 0.0:
                                    zero_weight_count += 1
                    ...
                    # -------- CP_Volume --------
                    pV = el.LookupParameter("CP_Volume")
                    if pV is not None:
                        try:
                            has_v = pV.HasValue
                        except Exception:
                        ...
                        if not has_v:
                            ...
                            try:
                                vf = float(pV.AsDouble())
                            except Exception:
                                null_volume_count += 1
                            else:
                                sum_v_ft3 += vf

                # CP_Weight is stored in kg, CP_Volume is stored in ft^3
                sum_w_lb = sum_w_kg / KG_PER_LB
                sum_v_gal = sum_v_ft3 / GAL_TO_FT3
                water_lb = sum_v_ft3 * 62.4
                total_lb = sum_w_lb + water_lb

                lines = []
                lines.append("REPORT (Scope: {}, Only owned: {})".format(
                    self._req.scope,
                    "YES" if self._req.only_owned else "NO"
                ))
                lines.append("Elements counted: {}".format(total))
                lines.append("")
                lines.append("Sum CP_Weight: {:,.2f} lb".format(sum_w_lb))
                lines.append("")
                lines.append("Sum CP_Volume: {:,.2f} gal".format(sum_v_gal))
                lines.append("")
                lines.append("Water weight (62.4 lb/ft³): {:,.2f} lb".format(water_lb))
                lines.append("")
                lines.append("TOTAL weight (CP_Weight + Water): {:,.2f} lb".format(total_lb))
                lines.append("")
                lines.append("NOTE: Totals are computed only from the current parameter values on elements.")
                lines.append("      Run the tool (Run) to populate/update CP_Weight and CP_Volume for accurate results.")
                lines.append("")
                lines.append("WARNINGS:")
                lines.append("  CP_Weight = 0 lb: {}".format(zero_weight_count))
                lines.append("  CP_Weight is NULL: {}".format(null_weight_count))
                lines.append("  CP_Volume is NULL: {}".format(null_volume_count))

                self._win.set_status_lines(lines)
                self._win.set_progress("Report complete", total, total)
                return
...
<Window.Resources>
  ...
  <Button x:Name="BtnHelpStatusMain" Content="?" Width="18" Height="18"
    ...
    <StackPanel Grid.Row="7" Orientation="Horizontal" HorizontalAlignment="Right" VerticalAlignment="Bottom" Margin="0,12,0,0">
      ...
      <Button x:Name="BtnReport" Content="Report" Width="110" Height="32" Margin="0,0,10,0" Style="{StaticResource SecondaryBtn}" />
...
class BatchWindow(object):
    def __init__(self):
        ...
        self.BtnReport = self._w.FindName("BtnReport")
        ...
        self.BtnReport.Click += self._on_report
...
Buttons:
- Run: Calculates and WRITES CP_Weight / CP_Volume / CP_BOM_Category to elements (based on your scope).

- Report: Reads existing parameter values only (NO writes). Use after Run to confirm totals.

- Stop Sync: Requests cancel. If a sync was selected, it will be skipped when cancelled.

- Model Setup: Opens the setup tool to import schedules and install/bind shared parameters.
...
If something fails, copy the Status text and send it to BIM/VDC.""",
    ...
    def _on_report(self, sender, args):
        try:
            if RUN_STATE.is_running:
                self.ProgText.Text = "Already running... click Stop Sync."
                return

            # Report uses the same scope + owned filter, but does NOT write parameters.
            self._req.report_only = True
            self._req.do_weight = False
            self._req.do_volume = False
            self._req.do_bomcat = False
            self._req.only_owned = bool(self.ChkOwned.IsChecked)
            self._req.do_sync = False
            self._req.scope = self._get_scope()

            RUN_STATE.cancel_requested = False
            self.set_progress("Queued report...", 0, 100)
            self.set_status_lines(["Queued report..."])
            do_events()
