...
HELP_OUTPUTS = (
    ...
    "• New Scope: sets Scope_NEW_MTI = Yes. Skipped if eM_CP_Override Parameter Sync is checked.\n"
    "• Micron Zone: writes Scope_Const_Work_Pkg_Ext_MTI for Micron construction zones.\n\n"
    "Optional outputs (normally leave OFF):\n"
    "• Future Scope: sets Scope_FUTURE_MTI = Yes. Skipped if eM_CP_Override Parameter Sync is checked — "
    "those elements must be set manually in the model.\n"
    "• Demo/Record: force those scope fields to No. Leaving them blank/null is faster for Sync with Central. "
...
class BatchWriteHandler(IExternalEventHandler):
    ...
    def Execute(self, uiapp):
        ...
        try:
                ...
                for start in range(0, total, CHUNK_SIZE):
                    ...
                    try:
                        for el in chunk:
                            ...
                            # New scope - Yes (skip if override is checked)
                            if self._req.do_scope_new_yes:
                                if _param_is_yes(el, "eM_CP_Override Parameter Sync"):
                                    n_skip += 1
                                elif set_param_int(el, "Scope_NEW_MTI", 1):
                                    n_ok += 1
                                else:
                                    n_skip += 1

                            # Future scope - Yes (skip if override is checked)
                            if self._req.do_future_yes:
                                if _param_is_yes(el, "eM_CP_Override Parameter Sync"):
                                    f_skip += 1
                                elif set_param_int(el, "Scope_FUTURE_MTI", 1):
                                    f_ok += 1
                                else:
                                    f_skip += 1

                            # Demo - No (skip if override is checked)
                            if self._req.do_demo_no:
                                if _param_is_yes(el, "eM_CP_Override Parameter Sync"):
                                    d_skip += 1
                                elif set_param_int(el, "Scope_DEMO_MTI", 0):
                                    d_ok += 1
                                else:
                                    d_skip += 1

                            # Record - No (skip if override is checked)
                            if self._req.do_record_no:
                                if _param_is_yes(el, "eM_CP_Override Parameter Sync"):
                                    r_skip += 1
                                elif set_param_int(el, "Scope_RECORD_MTI", 0):
                                    r_ok += 1
                                else:
                                    r_skip += 1
                            ...
                            # Construction Status (no override guard - handled by its own rules)
            ...
            if self._req.do_scope_new_yes:
                lines += ["", "Scope_NEW_MTI (override-guarded)", "  wrote:   {}".format(n_ok), "  skipped: {}".format(n_skip)]
            if self._req.do_future_yes:
                lines += ["", "Scope_FUTURE_MTI (override-guarded)", "  wrote:   {}".format(f_ok), "  skipped: {}".format(f_skip)]
            if self._req.do_demo_no:
...
• Job Number: writes Project Number_MTI = B111.
• New Scope: sets Scope_NEW_MTI = Yes. Skipped if override is checked.
• Micron Zone: writes Scope_Const_Work_Pkg_Ext_MTI for Micron construction zones.

Optional outputs (normally leave OFF):
• Future Scope: sets Scope_FUTURE_MTI = Yes. Skipped if override is checked (set manually in model).
• Demo/Record: forces those scope fields to No (blank/null = faster sync). Turn ON only to force Yes back to No.\"/></Button.ToolTip></Button></StackPanel>

        <CheckBox x:Name=\"ChkJob\"   Content=\"Write Job Number (Project Number_MTI = B111)\" IsChecked=\"True\" Margin=\"0,2,0,2\"/>
        <CheckBox x:Name=\"ChkNew\"   Content=\"Write New Scope - Yes (Scope_NEW_MTI = Yes, override-guarded)\" IsChecked=\"True\" Margin=\"0,2,0,2\"/>
...
• Force AB: after laser scan + model updates (As-Built).\"/></Button.ToolTip></Button></StackPanel>
        ...
        <StackPanel Orientation=\"Vertical\" Margin=\"12,0,0,0\">
          <CheckBox x:Name=\"ChkFuture\" Content=\"Write Future Scope - Yes (Scope_FUTURE_MTI = Yes, override-guarded)\" IsChecked=\"False\" Margin=\"0,2,0,2\"/>
...
class BatchWindow(object):
    ...
    def _on_run(self, sender, args):
        try:
            ...
            self._req.do_future_yes = bool(self.ChkFuture.IsChecked)
