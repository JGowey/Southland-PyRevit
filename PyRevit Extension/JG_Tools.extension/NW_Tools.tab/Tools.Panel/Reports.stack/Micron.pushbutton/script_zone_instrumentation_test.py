...
HELP_OUTPUTS = (
    ...
    "These force specific Micron scope fields to No. Leaving them blank/null is faster for Sync with Central. "
    "Turn ON only if you need to force any Yes values back to No."
)

HELP_WRITE_STATUS = (
    "Auto (recommended): keeps existing value; blanks become WIP. If it is already IFF or AB, it stays the same.\n\n"
    "Force WIP: use while modeling (before area sign-off).\n"
    "Force IFF: after GC + Micron review/sign-off (Issue for Fabrication).\n"
    "Force AB: after laser scan + model updates (As-Built)."
)

HELP_OPTIONS = (
    "Only elements I own: limits writing to elements currently checked out by you (workshared models).\n"
    "Sync with Central: syncs + relinquishes at the end to help keep the model clean."
)

HELP_SCOPE = (
    "Current View: elements visible in the active view.\n"
    "Entire Model: all matching model elements.\n"
    "Current Selection: only the elements you have selected."
)

HELP_STATUS_PANEL = (
    "Shows progress and live messages while the tool runs.\n"
    "Idle means nothing is running. Use Stop to safely end a run early."
)
...
# --- Micron Zone instrumentation (TESTING) ---
ZONE_STATS = {
    "calls": 0,
    "override_returned": 0,
    "center_hit": 0,
    ...
    "fallback_hit": 0,
    "no_zone": 0,
    "center_zone_tests": 0,     # total zone bbox tests during center stage
    "fallback_zone_tests": 0,   # total zone bbox tests during fallback stage
    "time_sec_total": 0.0,
    "time_sec_center": 0.0,
    "time_sec_fallback": 0.0,
    "max_center_tests": 0,
    "max_fallback_tests": 0,
}

def _reset_zone_stats():
    try:
        for k in list(ZONE_STATS.keys()):
            if isinstance(ZONE_STATS[k], (int, long)):
                ZONE_STATS[k] = 0
            elif isinstance(ZONE_STATS[k], float):
                ZONE_STATS[k] = 0.0
            else:
                ZONE_STATS[k] = 0
    except:
...
def _zone_stats_report():
    # Returns a list of human-readable lines
    try:
        calls = float(ZONE_STATS.get("calls", 0) or 0)
        if calls <= 0:
            return ["Micron Zone (instrumentation): no calls recorded."]

        center_hit = float(ZONE_STATS.get("center_hit", 0) or 0)
        fallback_used = float(ZONE_STATS.get("fallback_used", 0) or 0)
        fallback_hit = float(ZONE_STATS.get("fallback_hit", 0) or 0)
        override_returned = float(ZONE_STATS.get("override_returned", 0) or 0)
        no_zone = float(ZONE_STATS.get("no_zone", 0) or 0)

        c_tests = float(ZONE_STATS.get("center_zone_tests", 0) or 0)
        f_tests = float(ZONE_STATS.get("fallback_zone_tests", 0) or 0)

        t_total = float(ZONE_STATS.get("time_sec_total", 0.0) or 0.0)
        t_center = float(ZONE_STATS.get("time_sec_center", 0.0) or 0.0)
        t_fallback = float(ZONE_STATS.get("time_sec_fallback", 0.0) or 0.0)

        def pct(x): return 100.0 * (x / calls) if calls else 0.0

        lines = []
        lines.append("Micron Zone (instrumentation):")
        lines.append("  calls: {}".format(int(calls)))
        lines.append("  override returned: {} ({:.1f}%)".format(int(override_returned), pct(override_returned)))
        lines.append("  center-hit: {} ({:.1f}%)".format(int(center_hit), pct(center_hit)))
        lines.append("  fallback used: {} ({:.1f}%)".format(int(fallback_used), pct(fallback_used)))
        if fallback_used:
            lines.append("    fallback-hit: {} ({:.1f}% of calls; {:.1f}% of fallback-used)".format(
                int(fallback_hit),
                pct(fallback_hit),
                (100.0 * (fallback_hit / fallback_used)) if fallback_used else 0.0
            ))
        lines.append("  not in zone: {} ({:.1f}%)".format(int(no_zone), pct(no_zone)))

        lines.append("  center tests (zone bbox checks): total {}, avg {:.1f}/call, max {}".format(
            int(c_tests), (c_tests / calls) if calls else 0.0, int(ZONE_STATS.get("max_center_tests", 0) or 0)
        ))
        lines.append("  fallback tests (zone bbox checks): total {}, avg {:.1f}/call, max {}".format(
            int(f_tests), (f_tests / calls) if calls else 0.0, int(ZONE_STATS.get("max_fallback_tests", 0) or 0)
        ))

        lines.append("  timing: total {:.2f}s, avg {:.4f}s/call".format(t_total, (t_total / calls) if calls else 0.0))
        lines.append("    center stage time: {:.2f}s".format(t_center))
        lines.append("    fallback stage time: {:.2f}s".format(t_fallback))
        return lines
    except Exception as ex:
        return ["Micron Zone (instrumentation): report failed: {}".format(ex)]
...
def calc_micron_zone(el, doc_):
    """Return Micron zone name for element.

    Instrumentation (TESTING):
      - counts center-hit vs fallback usage
      - counts zone bbox tests in each stage
      - times total/center/fallback
    """
    if not el or not doc_:
        return MSG_NOZONE

    t0 = time.time()
    ZONE_STATS["calls"] = int(ZONE_STATS.get("calls", 0) or 0) + 1
    ...
    try:
        if _param_is_yes(el, override_param):
            ...
            if ov and ov.strip():
                ZONE_STATS["override_returned"] = int(ZONE_STATS.get("override_returned", 0) or 0) + 1
                ZONE_STATS["time_sec_total"] = float(ZONE_STATS.get("time_sec_total", 0.0) or 0.0) + (time.time() - t0)
                return ov
    ...
    if not ZONE_CACHE or int(ZONE_CACHE.get("ZoneCount", 0)) <= 0:
        ZONE_STATS["no_zone"] = int(ZONE_STATS.get("no_zone", 0) or 0) + 1
        ZONE_STATS["time_sec_total"] = float(ZONE_STATS.get("time_sec_total", 0.0) or 0.0) + (time.time() - t0)
        return MSG_NOZONE
    ...
    if not pt:
        ZONE_STATS["no_zone"] = int(ZONE_STATS.get("no_zone", 0) or 0) + 1
        ZONE_STATS["time_sec_center"] = float(ZONE_STATS.get("time_sec_center", 0.0) or 0.0) + (time.time() - t_center0)
        ZONE_STATS["time_sec_total"] = float(ZONE_STATS.get("time_sec_total", 0.0) or 0.0) + (time.time() - t0)
        return MSG_NOZONE
    ...
    ZONE_STATS["center_zone_tests"] = int(ZONE_STATS.get("center_zone_tests", 0) or 0) + int(center_tests)
    if center_tests > int(ZONE_STATS.get("max_center_tests", 0) or 0):
        ZONE_STATS["max_center_tests"] = int(center_tests)
    ZONE_STATS["time_sec_center"] = float(ZONE_STATS.get("time_sec_center", 0.0) or 0.0) + (time.time() - t_center0)

    if center_candidates:
        center_candidates.sort(key=lambda x: x[0])  # smallest vol wins
        ZONE_STATS["center_hit"] = int(ZONE_STATS.get("center_hit", 0) or 0) + 1
        ZONE_STATS["time_sec_total"] = float(ZONE_STATS.get("time_sec_total", 0.0) or 0.0) + (time.time() - t0)
        return center_candidates[0][1]

    # ---- 2) FALLBACK: ELEMENT BBOX INTERSECTS ZONE (XYZ) ----
    ZONE_STATS["fallback_used"] = int(ZONE_STATS.get("fallback_used", 0) or 0) + 1
    t_fb0 = time.time()
    fb_tests = 0
    ...
    if not ebb:
        ZONE_STATS["no_zone"] = int(ZONE_STATS.get("no_zone", 0) or 0) + 1
        ZONE_STATS["time_sec_fallback"] = float(ZONE_STATS.get("time_sec_fallback", 0.0) or 0.0) + (time.time() - t_fb0)
        ZONE_STATS["time_sec_total"] = float(ZONE_STATS.get("time_sec_total", 0.0) or 0.0) + (time.time() - t0)
        return MSG_NOZONE
    ...
    ZONE_STATS["fallback_zone_tests"] = int(ZONE_STATS.get("fallback_zone_tests", 0) or 0) + int(fb_tests)
    if fb_tests > int(ZONE_STATS.get("max_fallback_tests", 0) or 0):
        ZONE_STATS["max_fallback_tests"] = int(fb_tests)
    ZONE_STATS["time_sec_fallback"] = float(ZONE_STATS.get("time_sec_fallback", 0.0) or 0.0) + (time.time() - t_fb0)

    if intersect_candidates:
        intersect_candidates.sort(key=lambda x: x[0])  # smallest vol wins
        ZONE_STATS["fallback_hit"] = int(ZONE_STATS.get("fallback_hit", 0) or 0) + 1
        ZONE_STATS["time_sec_total"] = float(ZONE_STATS.get("time_sec_total", 0.0) or 0.0) + (time.time() - t0)
...
class BatchWriteHandler(IExternalEventHandler):
    ...
    def Execute(self, uiapp):
        ...
        # reset Micron Zone instrumentation each run
        _reset_zone_stats()
        ...
        try:
            ...
            if total == 0:
                # If Sync with Central is checked, sync even when nothing passes the filter.
                if self._req.do_sync:
                    self._win.set_progress("Syncing with Central...", 0, 0)
                    self._win.set_status_lines([
                        "Scope: {}".format(self._req.scope),
                        "Only owned: {}".format("YES" if self._req.only_owned else "NO"),
                    ...
                    self._win.set_status_lines([
                        ...
                        "Processed: 0 / 0",
                        "Stopped: NO",
                        "Elapsed: {:.1f} sec".format(time.time() - start_time),
                        "",
                        "Sync with Central: {}".format(sync_result),
                    ])
                    self._win.set_progress("Done", 0, 0)
                else:
...
• Current Selection: only the elements you have selected.\"/></Button.ToolTip></Button></StackPanel>
    ...
    <Border Grid.Row=\"4\" Style=\"{StaticResource Card}\">
      <Grid>
        ...
        <StackPanel Grid.Row=\"0\" Orientation=\"Horizontal\" VerticalAlignment=\"Center\" Margin=\"0,0,0,8\"><TextBlock Text=\"Status\" FontSize=\"15\" FontWeight=\"SemiBold\"/><Button x:Name=\"HelpStatus\" Content=\"?\" Style=\"{StaticResource HelpBtn}\"><Button.ToolTip><TextBlock TextWrapping=\"Wrap\" Width=\"420\" Text=\"Status panel:
Shows progress and live messages while the tool runs.
Idle means nothing is running. Use Stop to safely end a run early.\"/></Button.ToolTip></Button></StackPanel>
        <ProgressBar Grid.Row=\"1\" x:Name=\"Prog\" Height=\"16\" Minimum=\"0\" Maximum=\"100\" Value=\"0\" Margin=\"0,0,0,10\"/>
        <TextBlock Grid.Row=\"2\" x:Name=\"ProgText\" Text=\"Idle\" Margin=\"0,0,0,8\"/>

        <TextBox Grid.Row=\"3\" x:Name=\"StatusText\"
...
class BatchWindow(object):
    def __init__(self):
        ...
        # Help buttons
        self.HelpOutputs = self._w.FindName("HelpOutputs")
        self.HelpWriteStatus = self._w.FindName("HelpWriteStatus")
        self.HelpOptions = self._w.FindName("HelpOptions")
        self.HelpScope = self._w.FindName("HelpScope")
        self.HelpStatus = self._w.FindName("HelpStatus")
        ...
        # Help popups (click the ?)
        try:
            self.HelpOutputs.Click += lambda s, e: self._show_help("Outputs", HELP_OUTPUTS)
            self.HelpWriteStatus.Click += lambda s, e: self._show_help("Construction Status", HELP_WRITE_STATUS)
            self.HelpOptions.Click += lambda s, e: self._show_help("Options", HELP_OPTIONS)
            self.HelpScope.Click += lambda s, e: self._show_help("Scope", HELP_SCOPE)
            self.HelpStatus.Click += lambda s, e: self._show_help("Status", HELP_STATUS_PANEL)
        except:
            pass

    def _show_help(self, title, msg):
        """UI-only help popup for the ? buttons."""
        # IMPORTANT: Revit modeless WPF windows can cause dialogs (TaskDialog/MessageBox)
        # to appear *behind* the tool window. Force an owned, topmost modal popup.
        try:
            import clr
            clr.AddReference("PresentationFramework")
            from System.Windows import Window, TextWrapping, Thickness
            from System.Windows import MessageBox, MessageBoxButton, MessageBoxImage

            # Bring our window forward before showing help
            try:
                self._w.Activate()
                prev_topmost = self._w.Topmost
                self._w.Topmost = True
            except:
                prev_topmost = None

            try:
                # Owned MessageBox stays on top of the owner window
                MessageBox.Show(self._w, msg, title, MessageBoxButton.OK, MessageBoxImage.Information)
            finally:
                try:
                    if prev_topmost is not None:
                        self._w.Topmost = prev_topmost
                except:
                    pass
        except:
    ...
    def set_progress(self, phase, value, maximum):
        try:
            ...
            # If maximum is 0 we use an indeterminate bar *only* for active phases.
            # For terminal/idle phases we must explicitly reset the bar so it stops animating.
            if maximum == 0:
                if phase in ("Done", "Idle", "Error"):
                    self.Prog.IsIndeterminate = False
                    self.Prog.Minimum = 0
                    self.Prog.Maximum = 100.0
                    self.Prog.Value = 0.0 if phase != "Done" else 100.0
                else:
