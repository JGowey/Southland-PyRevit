...
def rotate_selected_element():
    """Rotate the selected Revit elements clockwise as a unit using the user's settings."""
    selection_ids = uidoc.Selection.GetElementIds()
    if not selection_ids:
        TaskDialog.Show("Error", "No element selected. Please select one or more instances in Revit.")
        return

    # Get the rotation axis direction based on the active view
    ...
    # Correct the angle for clockwise rotation based on the view type.
    active_view = doc.ActiveView
    
    # For Plan and Section views, the rotation direction is inverted.
    # We need to apply a negative angle for clockwise.
    if isinstance(active_view, ViewPlan) or isinstance(active_view, ViewSection):
        angle_degrees = -angle_degrees
    # For 3D views, a positive angle is clockwise when looking towards the origin.
    ...
    # --- NEW: compute combined bounding box for the whole selection ---
    has_bbox = False
    ...
    # Center of combined bounding box = rotation point for the whole set
    rotation_point = XYZ(
        (min_x + max_x) / 2.0,
        (min_y + max_y) / 2.0,
        (min_z + max_z) / 2.0
    ...
    t = Transaction(doc, "Rotate Selection Clockwise")
    t.Start()
    try:
...
def rotate_family_instance(element, angle_radians, view_forward):
    ...
    t = Transaction(doc, "Rotate RFA Clockwise")
    t.Start()
    try:
...
def rotate_itm(element, angle_radians, view_forward):
    ...
    t = Transaction(doc, "Rotate ITM Clockwise")
    t.Start()
    try:
        ElementTransformUtils.RotateElement(doc, element.Id, axis, angle_radians)
        t.Commit()
