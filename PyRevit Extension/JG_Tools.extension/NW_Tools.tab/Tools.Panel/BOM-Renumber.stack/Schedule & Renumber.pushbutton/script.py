...
def build_rfa_list(instances):
    ...
    for eid in rfa_ids:
        ...
        if isinstance(fi, FamilyInstance) and not is_gtp_family(fi):
            rfas.append(fi)
...
with Transaction(doc, "Populate BOM + Renumber") as t:
    ...
    # Note: set_item_number() will silently skip elements without writable parameters
    n = 1
    if fab_sorted:
        n = renumber_fabrication(fab_sorted, n)
    if native_sorted:
        n = renumber_native(native_sorted, n)
    if rfas_sorted:
