# -*- coding: utf-8 -*-
"""
Southland NW VDC - BOM Population & Stable Renumbering Tool
============================================================
Version: 1.0
...
OVERVIEW:
    Automated BOM population and item numbering for MEP and structural elements.
    Handles fabrication parts, native Revit MEP, and family instances with
    deterministic, stable numbering across multiple runs.

SUPPORTED ELEMENTS:
    • MEP Fabrication Parts (Pipework, Ductwork, Hangers)
    • Native Revit MEP (Pipes, Ducts, Flex, Fittings)
    • Family Instances (Structural Framing, Conduit Fittings, etc.)

WORKFLOW:
    1. User selects elements
    2. Elements classified by type
    3. Elements sorted by phase/category
    4. BOM parameters populated
    5. Item numbers assigned (grouped by similarity)
    6. Changes committed to model

FABRICATION NUMBERING PHASES:
    Phase 1: Straights (pipes/ducts with 21FT-/40FT- prefix)
    Phase 2: Fishmouths (CID 2875)
    Phase 3: Fittings (couplings, valves, elbows, etc.)
    Phase 4: Taps
...
SPECIAL HANDLING:
    • Pipe prefixes: ≤21' → 21FT-, >21' → 40FT-
    • Fishmouths: Phase 2, length populated
    • Welds: Individual numbering, excludes weld neck flanges
    • Nested families: Collected recursively from selection
    • Conduit fittings: Sort with structural framing
"""
...
# Step 4-6: Execute
with Transaction(doc, "Populate BOM + Renumber") as t:
    t.Start()
