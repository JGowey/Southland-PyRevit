# -*- coding: utf-8 -*-
"""
Southland NW VDC - Item Renumbering Tool
=========================================
Version: 1.0 (Renumber Only)
Author: Southland NW VDC Team
Date: 2026-02-17

OVERVIEW:
    Renumbers MEP and structural elements WITHOUT updating BOM parameters.
    Identical pipeline to the full script — selection, classification, sorting,
    and grouping are unchanged. BOM population (Step 4) has been removed.

WHAT THIS SCRIPT UPDATES:
    ✓ CP_Item Number
    ✓ Item Number
    ✓ ItemNumber (fabrication-specific)
...
WHAT THIS SCRIPT DOES NOT TOUCH:
    ...
    ✗ CP_Length
    ✗ CP_Material & Coating
    ✗ CP_Fabrication_Connector_Name_1/2
...
WORKFLOW:
    ...
    3. Elements sorted by phase/category
    4. Elements grouped by properties (uses existing BOM descriptions)
    5. Item numbers assigned (grouped elements get same number)
    6. Changes committed to model

HOW RENUMBERING WORKS:
    The script uses EXISTING BOM parameters (CP_BOM_Description, CP_Size, etc.)
    to determine grouping. Elements with identical properties get the same
    item number. Sorting determines the ORDER in which numbers are assigned.
    
    THIS MEANS: BOM parameters must already be populated for correct grouping!
    Run the BOM update script first if descriptions/sizes have changed.

FABRICATION NUMBERING PHASES:
    Phase 1: Straights (sorted by description prefix 21FT-/40FT-)
    Phase 2: Fishmouths (CID 2875)
    Phase 3: Fittings (couplings, valves, elbows, tees, etc.)
    Phase 4: Taps
    Phase 5: Welds (individual W1, W2, W3... numbers)
    Phase 6: Flex

GROUPING LOGIC:
    Elements with identical properties get the same item number:
    
    Fabrication: Description + Material + CID + Size + Length (±1/16")
    Native MEP:  Category + Description + Size + Length (±1/16")
    RFAs:        Category + Family + Type + Dimensions
"""
...
ITEMNUMBER_PARAM_NAMES = ("ItemNumber", "Item Number")
...
def build_rfa_list(instances):
    ...
    for fi in instances:
        ...
        # Skip if no target parameter
        if not get_param(fi, TARGET_PARAM):
            continue
    ...
    for eid in rfa_ids:
        ...
        if isinstance(fi, FamilyInstance) and not is_gtp_family(fi) and get_param(fi, TARGET_PARAM):
            rfas.append(fi)
...
def fab_group_key(fp):
    """Creates grouping key for fabrication parts."""
    desc = (fp.ProductLongDescription or "").strip()
    mat = get_fab_material(fp)
    cid = getattr(fp, "ItemCustomId", 0)
    size = get_fab_size(fp)
    
    try:
        length = fp.CenterlineLength
        norm_length = normalize_length(length) if length is not None else None
    except:
        norm_length = None
    
    return (desc, mat, cid, size, norm_length)


def fab_sort_key(fp):
    """Creates sort key for fabrication parts."""
    try:
        cat = fp.Category.Name if fp.Category else ""
    except:
        cat = ""
    ...
    desc = (fp.ProductLongDescription or "").strip()
    cid = getattr(fp, "ItemCustomId", 0)
    diam = get_param_double(fp, DIAM_PARAM, 0.0)
    ...
    return (cat, phase, desc, cid, -diam, -length, fp.Id.IntegerValue)


def native_group_key(el):
    """Creates grouping key for native MEP."""
    try:
    ...
    norm_length = normalize_length(length) if length else None
    
    return (cat, desc, size, norm_length)


def native_sort_key(el):
    """Creates sort key for native MEP."""
    try:
        cat = el.Category.Name if el.Category else ""
    except:
        cat = ""
    ...
    desc = get_native_description(el)
    length = get_native_length(el) or 0.0
    
    return (cat, desc, -length, el.Id.IntegerValue)


def rfa_group_key(fi):
    """Creates grouping key for RFAs (groups by type NAME, not ID)."""
    return (
        get_rfa_category(fi),
        get_rfa_family(fi),
        get_rfa_type(fi),  # Use type NAME for proper grouping
        get_rfa_length(fi, "CP_Length"),
...
def rfa_sort_key(fi):
    """Creates sort key for RFAs (conduit fittings sort with structural)."""
    cp_len = get_rfa_length(fi, "CP_Length")
    cp_len_sort = cp_len if cp_len is not None else 1e99
    
    # Remap conduit fittings to sort with structural
    cat = get_rfa_category(fi)
    if cat == "Conduit Fittings":
        cat = "Structural Framing"
    
    return (cat, get_rfa_family(fi), get_rfa_type(fi), cp_len_sort, fi.Id.IntegerValue)
...
def set_item_number(el, value):
    """Sets item number on element."""
    # CP_Item Number
    ...
    # Fabrication-specific parameters
    if isinstance(el, FabricationPart):
        for name in ITEMNUMBER_PARAM_NAMES:
            p = get_param(el, name)
            if p and not p.IsReadOnly:
                try:
                    p.Set(str(value))
...
def build_groups(items, key_func, filter_func=None):
    """Groups items by key function."""
    groups = {}
    ordered_keys = []
...
def assign_sequential(ordered_keys, groups, start):
    """Assigns sequential numbers to groups."""
    n = start
    for key in ordered_keys:
        for item in groups.get(key, []):
            set_item_number(item, str(n))
        n += 1
    return n


def assign_welds(items, start):
    """Assigns individual weld numbers."""
    w = start
    for item in items:
        set_item_number(item, "W{}".format(w))
        w += 1
    return w


def renumber_fabrication(sorted_items, start):
    """Renumbers fabrication parts by phase."""
    n = start
    w = 1
    
    for phase in [Phase.STRAIGHT, Phase.FISHMOUTH, Phase.FITTING, Phase.TAP, Phase.WELD, Phase.FLEX]:
        if phase == Phase.WELD:
            # Welds: individual numbers
            welds = [fp for fp in sorted_items if classify_fab_phase(fp) == phase]
            w = assign_welds(welds, w)
        else:
            # Other phases: grouped numbers
            keys, groups = build_groups(
                sorted_items,
                fab_group_key,
                lambda fp: classify_fab_phase(fp) == phase
            )
            n = assign_sequential(keys, groups, n)
    
    return n


def renumber_native(sorted_items, start):
    """Renumbers native MEP elements."""
    keys, groups = build_groups(sorted_items, native_group_key)
    return assign_sequential(keys, groups, start)


def renumber_rfas(sorted_items, start):
    """Renumbers family instances."""
    keys, groups = build_groups(sorted_items, rfa_group_key)
    return assign_sequential(keys, groups, start)
...
# Step 3: Sort
fab_sorted = sorted(fabrication, key=fab_sort_key)
native_sorted = sorted(native_mep, key=native_sort_key)
rfas_sorted = build_rfa_list(rfas)
rfas_sorted = sorted(rfas_sorted, key=rfa_sort_key)

# Exit if nothing
if not fab_sorted and not native_sorted and not rfas_sorted:
    script.exit()

# Step 4-5: Execute Renumbering
with Transaction(doc, "Renumber Elements") as t:
    t.Start()
    
    # Step 4: Clear Numbers
    for fp in fab_sorted:
        set_item_number(fp, "")
    for el in native_sorted:
        set_item_number(el, "")
    for fi in rfas_sorted:
        set_item_number(fi, "")
    
    # Step 5: Assign Numbers
    fab_with_param = [fp for fp in fab_sorted if get_param(fp, TARGET_PARAM)]
    native_with_param = [el for el in native_sorted if get_param(el, TARGET_PARAM)]
    rfas_with_param = [fi for fi in rfas_sorted if get_param(fi, TARGET_PARAM)]
    
    n = 1
    if fab_with_param:
        n = renumber_fabrication(fab_with_param, n)
    if native_with_param:
        n = renumber_native(native_with_param, n)
    if rfas_with_param:
        n = renumber_rfas(rfas_with_param, n)
