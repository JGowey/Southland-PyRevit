...
class _LongSafeEncoder(json.JSONEncoder):
    """IronPython 2.7 returns ElementId.Value as long (e.g. 1151091L).
    Python 2's json encoder doesn't handle long -- coerce to int."""
    def default(self, obj):
        import sys
        if sys.version_info[0] == 2 and isinstance(obj, long):  # noqa: F821
            return int(obj)
        return super(_LongSafeEncoder, self).default(obj)


def save_profile(name, data):
    _ensure_profiles_dir()
    with open(os.path.join(PROFILES_DIR, name + ".json"), "w") as fp:
        json.dump(data, fp, indent=2, cls=_LongSafeEncoder)
