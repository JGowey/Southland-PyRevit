...
class UpdateSleeveWindow(object):
    ...
    def __init__(self, xaml_file, config, source_elem, source_data):
        ...
        # Restore radius from config before first scan
        self.RadiusBox.Text = str(config.get("radius_ft", ""))
        self._run_scan()
    ...
    def _get_controls(self):
        ...
        self.RadiusBox            = f("RadiusBox")
        self.BtnDeselectAll       = f("BtnDeselectAll")
        self.BtnRescan            = f("BtnRescan")
    ...
    def _wire_events(self):
        ...
        self.BtnDeselectAll.Click         += lambda s, e: self.DeselectAll_Click(s, e)
        self.BtnRescan.Click              += lambda s, e: self.Rescan_Click(s, e)
        f = self._window.FindName
    ...
    def Import_Click(self, sender, e):
        ...
        if dlg.ShowDialog() == System.Windows.Forms.DialogResult.OK:
            try:
                ...
                self.RadiusBox.Text   = str(cfg.get("radius_ft", ""))
                self._update_preview()
    ...
    def _run_scan(self):
        ...
        # Radius filter:
        #   blank = no limit (show all sleeves)
        #   0     = sleeves co-located with content (tight 1ft tolerance)
        #   N     = sleeves within N feet in the perpendicular plane
        raw_radius = self.RadiusBox.Text.strip()
        if raw_radius == "":
            radius_ft = None   # no limit
        else:
            try:
                val = float(raw_radius)
                radius_ft = 1.0 if val == 0.0 else val  # 0 = 1ft tight tolerance
            except Exception:
                radius_ft = None

        # ── Step 1: Determine curve orientation ────────────────────
        # Get source centerpoint and normalized flow direction vector
        src_pt   = None
        flow_vec = None  # (fx, fy, fz) unit vector along pipe axis
        ...
        if hasattr(loc, "Curve"):
            ...
            p0 = c.GetEndPoint(0)
            p1 = c.GetEndPoint(1)
            src_pt = DB.XYZ((p0.X + p1.X) / 2.0,
                             (p0.Y + p1.Y) / 2.0,
                             (p0.Z + p1.Z) / 2.0)
            raw    = (p1.X - p0.X, p1.Y - p0.Y, p1.Z - p0.Z)
            length = (raw[0]**2 + raw[1]**2 + raw[2]**2) ** 0.5
            if length > 0.001:
                flow_vec = (raw[0]/length, raw[1]/length, raw[2]/length)
        elif hasattr(loc, "Point"):
            src_pt = loc.Point

        # ── Step 2: Build two perpendicular axes to pipe flow ───────
        # Given unit flow vector F, find two orthogonal unit vectors U, V
        # such that U and V span the cross-section plane.
        # Any vector not parallel to F works as a seed.
        perp_u = None
        perp_v = None
        if flow_vec is not None:
            fx, fy, fz = flow_vec
            # Pick a seed vector not parallel to flow
            if abs(fx) <= abs(fy) and abs(fx) <= abs(fz):
                ...
                seed = (0.0, 0.0, 1.0)
            # U = seed - (seed.F)F  (Gram-Schmidt)
            dot = seed[0]*fx + seed[1]*fy + seed[2]*fz
            ux = seed[0] - dot*fx
            uy = seed[1] - dot*fy
            uz = seed[2] - dot*fz
            umag = (ux*ux + uy*uy + uz*uz) ** 0.5
            if umag > 0.0001:
                perp_u = (ux/umag, uy/umag, uz/umag)
                # V = F cross U
                vx = fy*perp_u[2] - fz*perp_u[1]
                vy = fz*perp_u[0] - fx*perp_u[2]
                vz = fx*perp_u[1] - fy*perp_u[0]
                perp_v = (vx, vy, vz)

        CRASH_LOG.log("=== Radius Setup ===")
        CRASH_LOG.log("  src_pt=({0:.3f},{1:.3f},{2:.3f})".format(
            src_pt.X, src_pt.Y, src_pt.Z) if src_pt else "  src_pt=None")
        CRASH_LOG.log("  flow_vec={0}".format(flow_vec))
        CRASH_LOG.log("  perp_u={0}  perp_v={1}".format(perp_u, perp_v))
        CRASH_LOG.log("  radius_ft={0}".format(radius_ft))

        for elem in scan_sleeves(CRASH_LOG.log, self._active_rule,
                                  self._source_data.get('shape')):
            # ── Step 3: 2D circle query in perpendicular plane ───────
            if radius_ft is not None and src_pt is not None:
                skip = False
                ep = None
                eloc = elem.Location
                if hasattr(eloc, "Point"):
                    ep = eloc.Point
                elif hasattr(eloc, "Curve"):
                    c2 = eloc.Curve
                    q0, q1 = c2.GetEndPoint(0), c2.GetEndPoint(1)
                    ep = DB.XYZ((q0.X+q1.X)/2.0,
                                 (q0.Y+q1.Y)/2.0,
                                 (q0.Z+q1.Z)/2.0)
                if ep is not None:
                    # Project delta onto the two perp axes
                    dx = ep.X - src_pt.X
                    dy = ep.Y - src_pt.Y
                    dz = ep.Z - src_pt.Z
                    if perp_u is not None and perp_v is not None:
                        # 2D coords in cross-section plane
                        coord_u = dx*perp_u[0] + dy*perp_u[1] + dz*perp_u[2]
                        coord_v = dx*perp_v[0] + dy*perp_v[1] + dz*perp_v[2]
                        dist_ft = (coord_u**2 + coord_v**2) ** 0.5
                    else:
                        # No flow vec -- 3D fallback
                        dist_ft = (dx*dx + dy*dy + dz*dz) ** 0.5
                    CRASH_LOG.log("  sleeve {0}: ep=({1:.2f},{2:.2f},{3:.2f}) perp_dist={4:.3f}ft radius={5}ft -> {6}".format(
                        elem.Id.IntegerValue, ep.X, ep.Y, ep.Z,
                        dist_ft, radius_ft,
                        "PASS" if dist_ft <= radius_ft else "SKIP"))
                    if dist_ft > radius_ft:
                        skip = True
                else:
                    CRASH_LOG.log("  sleeve {0}: could not get location -- pass through".format(
        ...
        if n:
            if radius_ft is not None:
                label = "co-located (1ft)" if float(self.RadiusBox.Text.strip() or "1") == 0.0 else "{0}ft".format(radius_ft)
                msg = "{0} sleeve(s) found within {1}.".format(n, label)
            else:
                msg = "{0} sleeve(s) found.".format(n)
        else:
            if radius_ft is not None:
                msg = "No sleeves found. Try larger radius or clear field for no limit."
            else:
                msg = "No sleeves found -- family name must contain 'sleeve'."
    ...
    def DeselectAll_Click(self, sender, e):
        """Uncheck all sleeves in the grid."""
        for row in self._sleeve_rows:
            row.Selected = False
        self._refresh_grid()
        self._set_status("All sleeves deselected.")

    def Rescan_Click(self, sender, e):
        """Re-run sleeve scan with current radius and rule settings."""
        try:
            raw = self.RadiusBox.Text.strip()
            self._config["radius_ft"] = raw  # store as string to preserve blank/0 distinction
        except Exception:
            pass
        save_config(self._config)
        self._run_scan()
