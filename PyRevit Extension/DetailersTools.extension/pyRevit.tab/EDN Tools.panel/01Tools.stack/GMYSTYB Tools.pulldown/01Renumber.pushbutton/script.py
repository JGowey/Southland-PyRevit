    ...
    try:
        ...
        if launch_place_tags and place_tags_elements:
            try:
                ...
                # Find Place Tags folder -- supports numeric prefix (e.g. "03Place Tags.pushbutton")
                _gmystyb_dir = os.path.dirname(script_dir)
                _pt_folder = next(
                    (d for d in os.listdir(_gmystyb_dir)
                     if d.lower().endswith("place tags.pushbutton")),
                    "03Place Tags.pushbutton"
                )
                place_tags_dir = os.path.join(_gmystyb_dir, _pt_folder)
